const actions = {
    "fetch-all": async () => {
      return await fetch(`${import.meta.env.VITE_API_URL}/Employees`, {
        method: "GET",
      }).then((response) => response.json());
    },
    "fetch-one": async (payload) => {
        return await fetch(`${import.meta.env.VITE_API_URL}/Employees/${payload.id}`, {
            method: "GET",
        }).then((response) => response.json());
        }
  ...
  export default async function useEmployees(action = 'fetch-all', payload) {
